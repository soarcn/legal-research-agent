...
| P1.3 Database bootstrap | P1.2 | SQLAlchemy engine, Alembic migration, connection test |
| P1.4 `/v1/health` | P1.2–P1.3 | Process liveness without external dependency calls |
...
| P1.8 `/v1/ready` | P1.2–P1.7 | Conditional readiness for Postgres, Weaviate, configured model, embedder, and reranker |
...
| P2.2 Domain models | P0 | `LegalDocument`, `DocumentVersion`, `LegalSection`, `LegalPassage`, `SourceSnapshot`, `IngestionJob` |
| P2.3 Stable ID specification | P2.2 | Deterministic IDs with repeated-run tests |
| P2.4 PostgreSQL schema expansion | P1.3, P2.2 | Alembic migration for provenance/version/ingestion entities |
...
| Task | Depends on | Output and acceptance |
| --- | --- | --- |
| P3.1 Legal RAG Bench parser | P2.1–P2.2 | Pure parser returning normalized models |
| P3.2 Section-aware chunker | P2.3, P2.5 | No cross-section splits; offsets round-trip to source |
| P3.3 Embedding adapter | P1.7, P3.2 | BGE-M3 batches with fixed dimension/revision/settings |
| P3.4 Weaviate v1 schema | P1.2, P2.2 | Flat-vector baseline, BM25 fields, metadata filters, versioned collection |
| P3.5 Ingestion service and CLI | P2.4, P3.1–P3.4 | Job state, idempotency, isolated failures, report |
...
| P6.7 Reliability evaluation | P6.6 | Citation, unsupported-claim, abstention, schema metrics |
...
| P8.7 Holdout final run | P8.5–P8.6 | Explicitly authorized first/final 20-case holdout report; no subsequent v1 tuning |
